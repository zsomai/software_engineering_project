...
const signin = async (email, hasshedPassword) => {
...
const addToWishList = async (wishList, bookID, userID) => {
...
const addToCurrentlyReading = async (user, pageNum, totalPages, id) => {
...
const addToDone = async (user, bookID) => {
...
const getRecomended = async (user) => {
    let recentlyRead = user.finished.sort(() => 0.5 - Math.random()).slice(0, 3);
    let bookList = await Promise.all(recentlyRead.map((el) => {
        try {
    ...
    let recommendedList = await Promise.all(bookList.map(async (book) => {
        try {
            const bookList = await BookServices.getSimilarBooks(book.plot);
            let item = { "title": "Based on " + book.title + " by: " + book.author, "similarBooks": bookList };
            return item;
        }
        catch (err) {
            console.log(err);
...
const getPagesReadToday = (user) => {
    return user.habitHistory.filter(el => +el.date === +(new Date().setHours(0, 0, 0, 0)))[0].pagesRead;
};
const getNumOfBooksReadThisYear = (user) => {
    console.log(user.yearlyReport[0]);
    return user.yearlyReport.filter(el => el.year === (new Date().getFullYear() - 1))[0].booksRead;
};
const setGoals = async (userID, dailyGoal, yearlyGoal) => {
    const filter = { _id: userID };
    const update = {};
    if (dailyGoal) {
...
const getBooksReadThisYear = async (user) => {
    const date = new Date().getFullYear();
    const listOfIds = user.finished.filter(data => data.date.getFullYear() + 1 === date);
...
const removeWishlist = async (user, bookID) => {
    let newList = removeFromList(user.wishList, new Types.ObjectId(bookID));
    const update = { wishList: newList };
    await User.findByIdAndUpdate(user._id, update, function (err, docs) {
        if (err) {
            console.log(err);
        }
        else {
...
const removeReading = async (user, bookID) => {
...
const getCategoriesReadThisYear = async (user) => {
    const books = await getBooksReadThisYear(user);
    const counts = {};
    console.log(books);
    for (var book of books) {
        if (book.geners) {
            const json = JSON.parse(book.geners);
